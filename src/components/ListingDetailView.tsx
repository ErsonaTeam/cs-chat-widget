    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between mb-2">
          <h2 className="text-xl font-bold text-primary flex-1">{listing.name}</h2>
          {listing.averageReviewRating !== null && (
            <div className="flex items-center gap-1 bg-primary/10 px-2 py-1 rounded-lg ms-2 shrink-0">
              <svg className="w-4 h-4 text-accent" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="text-sm font-semibold text-primary">{listing.averageReviewRating.toFixed(1)}</span>
            </div>
          )}
        </div>

        {/* City */}
        <div className="flex items-center gap-1 text-sm text-primary/60 mb-3">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="w-full bg-primary hover:bg-primary/90 text-white font-semibold py-3 px-4 rounded-lg transition-colors shadow-md"
        >

    ...
    >
      ...
      <div className="relative h-40 bg-fattalCream">
        ...
        {listing.averageReviewRating !== null && (
          <div className="absolute top-2 end-2 bg-fattalNavy/80 text-white text-xs font-semibold px-2 py-1 rounded-lg flex items-center gap-1">
            <svg className="w-3 h-3 text-fattalGold" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="p-4">
        ...
        {/* Specs */}
        <div className="flex flex-wrap gap-2 text-xs text-fattalNavy/70 mb-2">
          <span>{listing.bedroomsNumber} {t(lang, 'bedrooms')}</span>
          <span className="text-fattalNavy/30">·</span>
        ...
        {/* Description snippet */}
        {listing.description && (
          <p className="text-xs text-fattalNavy/60 mb-3 line-clamp-2">
            {listing.description}
          </p>
        )}
